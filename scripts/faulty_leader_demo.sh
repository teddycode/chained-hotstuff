...
./examples/hotstuff-app --conf ./hotstuff-sec0.conf > log0.log 2>&1 &
leader_pid="$!"
rep=({1..3})
if [[ $# -gt 0 ]]; then
    rep=($@)
...
for i in "${rep[@]}"; do
    ...
    ./examples/hotstuff-app --conf ./hotstuff-sec${i}.conf > log${i}.log 2>&1 &
done
...
kill "$cli_pid"
./examples/hotstuff-client --idx 0 --iter -1 --max-async 4 > log_client.log 2>&1
